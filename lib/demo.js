...
function demoSearch({ q = 'angebote', minPrice, maxPrice, page = 1 }, force = false) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const createdAgoH = Math.floor(rnd() * 72);
    const d = new Date(clock() - createdAgoH * 3600e3);
    ...
    listings.push({
      ...
      dateTxt: createdAgoH < 24 ? 'Heute, ' + (8 + Math.floor(rnd() * 12)) + ':' + String(Math.floor(rnd() * 6)) + '5'
        : createdAgoH < 48 ? 'Gestern' : d.toISOString().slice(0, 10).split('-').reverse().join('.'),
...
function demoDetail(id) {
  ...
  return {
    ...
    date: new Date(clock() - Math.floor(rnd() * 72) * 3600e3).toISOString().slice(0, 10),
