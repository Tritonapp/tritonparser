...
const CACHE_TTL_MS = parseInt(process.env.KA_CACHE_TTL_MS || String(5 * 60 * 1000), 10);
...
async function get(url, { referer, ttlMs = CACHE_TTL_MS, force = false } = {}) {
  ...
  const task = (async () => {
    ...
    let attempt = 0;
    // до 2 попыток; на 403 делаем паузу и пробуем ещё раз
    while (true) {
      attempt++;
      ...
      try {
        ...
        if (attempt >= 2) throw new Error('network: ' + e.message);
        await sleep(6000 + Math.random() * 4000);
        continue;
      }
      if (out.status === 200 && out.html && out.html.length > 5000) {
      ...
      if (attempt >= 2) throw new Error('http:' + out.status);
      await sleep(4000);
    }
  })();
...
/** URL поиска. Примеры сегментов: preis:100:800 */
function searchUrl({ q, minPrice, maxPrice, page = 1 }) {
...
module.exports = { get, searchUrl, adUrl, stats, BASE };
