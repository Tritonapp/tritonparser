...
/**
 * Точный момент публикации (epoch ms) для свежих лотов:
 * "Heute, 16:43" — время берлинское (CEST +2 / CET +1), приводим к UTC;
 * "vor 25 Minuten" / "vor 3 Stunden" — относительное время выдачи.
 * Для "Gestern" и дат без времени точность недостижима -> null.
 */
function parsePostedAt(s, now = new Date()) {
  const t = decode(s || '');
  if (!t) return null;
  let m = t.match(/^heute[^0-9]*(\d{1,2}):(\d{2})/i);
  if (m) {
    const month = now.getUTCMonth() + 1;
    const off = (month >= 3 && month <= 10) ? 2 : 1;     // CEST / CET
    const shifted = new Date(now.getTime() + off * 3600e3); // «сегодня» по Берлину
    return Date.UTC(shifted.getUTCFullYear(), shifted.getUTCMonth(), shifted.getUTCDate(),
      parseInt(m[1], 10), parseInt(m[2], 10)) - off * 3600e3;
  }
  m = t.match(/vor\s+(\d+)\s+(minute?n?|stunden?|min)/i);
  if (m) {
    const n = parseInt(m[1], 10);
    return (now instanceof Date ? now.getTime() : Date.now()) - n * (/stunde/i.test(m[2]) ? 3600e3 : 60000);
  }
  return null;
}
...
/** Город из alt-подписи картинки: "...{TITLE} {Region} - {Ort} Vorschau" */
function locFromAlt(alt) {
  let s = String(alt || '').replace(/\s*Vorschau\s*$/, '').trim();
  const plz = s.match(/(\d{5}\s+\S[^"]{1,30})$/);          // "68229 Mannheim"
  if (plz) return plz[1].trim();
  const dash = s.lastIndexOf(' - ');
  if (dash > 0 && s.length - dash < 60) return s.slice(dash + 3).trim();
  return null;
}
...
function parseSearch(html) {
  ...
  for (const a of arts) {
    ...
    // город: 1) классическая раскладка, 2) новая (span после svg-иконки), 3) alt-подпись
    let location = null;
    const loc1 = b.match(/aditem-main--top--left">\s*(?:<i[^>]*>\s*<\/i>\s*)?([^<]+)</);
    if (loc1) location = decode(loc1[1]);
    if (!location) {
      const loc2 = b.match(/data-title="locationOutline"[\s\S]*?<\/svg>\s*<span[^>]*>([^<]+)<\/span>/);
      if (loc2) location = decode(loc2[1]);
    }
    if (!location) {
      const altM = b.match(/alt="([^"]*)"/);
      if (altM) location = locFromAlt(decode(altM[1]));
    }
    ...
    listings.push({
      ...
      postedAt: parsePostedAt(dateTxt || ''),
      image,
      imgCount,
      tags,
      isTop,
...
module.exports = { parseSearch, parseDetail, parsePrice, parseKaDate, parsePostedAt, decode, parseTotal };
