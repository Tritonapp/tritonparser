...
function parseDetail(html) {
  ...
  const since = html.match(/(seit|Mitglied seit)[^<]*<[^>]*>\s*(\d{1,2}\.\d{2}\.\d{4})/);
  if (since) seller.memberSince = since[2];
  ...
  const category = grab(/<meta property="og:title" content="[^"]*? in [^|]*?\|\s*([^|]+)\|/);

  return {
    title, price: pr.price, priceRaw: pr.raw, negotiable: pr.negotiable,
    date, description, details, images, seller, sold, category,
  };
}

module.exports = { parseSearch, parseDetail, parsePrice, parseKaDate, decode };
