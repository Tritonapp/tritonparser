...
function viewsMetaFor(id, postedAt, at) {
  const now = at || Date.now();
  const out = { postedAt: postedAt || null, ageMin: null, vpm: null, views: null, isHot: false, viewsReal: false };
...
/** Сохранить реальные просмотры лота (счётчик Kleinanzeigen) */
const REAL_VIEWS_FRESH_MS = 15 * 60e3;
function setRealViews(id, n, at) {
  const rec = state.ads[id];
  if (!rec) return;
  rec.viewsReal = { n, t: at || Date.now() };
  saveSoon();
}

/** Точный момент публикации лота из истории (или null) */
function postedAtFor(id) {
  const rec = state.ads[id];
  return rec ? (rec.postedAt || null) : null;
}

/** Свежие ли реальные просмотры у лота (если есть) */
function realViewsFor(id, now) {
  const rec = state.ads[id];
  if (!rec || !rec.viewsReal) return null;
  if ((now || Date.now()) - rec.viewsReal.t > REAL_VIEWS_FRESH_MS) return null;
  return rec.viewsReal.n;
}
...
function decorate(listings, now) {
  ...
  return listings.map(l => {
    ...
    // возраст лота (мин), просмотры и флаг «горячий» по правилу 20мин–4ч / 0.5 просм./мин;
    // если известны РЕАЛЬНЫЕ просмотры (счётчик KA) — используем их вместо оценки
    const vm = viewsMetaFor(l.id, l.postedAt, now);
    const realN = l.postedAt ? realViewsFor(l.id, now) : null;
    if (realN != null) {
      vm.views = realN;
      vm.vpm = Math.round((realN / Math.max(vm.ageMin, 1)) * 100) / 100;
      vm.isHot = vm.ageMin >= HOT_MIN_AGE_MIN
        && vm.ageMin <= HOT_MAX_AGE_MIN
        && realN / Math.max(vm.ageMin, 1) >= HOT_MIN_VPM;
      vm.viewsReal = true;
    }
    Object.assign(meta, vm);
...
module.exports = { recordSnapshot, decorate, trending, stats, historyFor, rememberRecent, recent, interestFor, vpmFor, viewsMetaFor, setRealViews, realViewsFor, postedAtFor };
