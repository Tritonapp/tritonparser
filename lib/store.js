 ...
 */
function interestFor(id, hot, firstSeen, priceDropPct, at) {
  const now = at || Date.now();
  const base = h32(id) % 14;                       // 0..13 — стартовая база лота
  const hours = Math.max(0, (now - (firstSeen || now)) / 3600e3);
  const rate = 0.9 + (hot || 0) / 50;              // горячие растут быстрее
  const dropBonus = (priceDropPct || 0) * 0.9;
  return Math.max(0, Math.round(base + (hot || 0) * 0.7 + hours * rate * 3 + dropBonus));
}
...
function decorate(listings, now) {
  ...
  return listings.map(l => {
    ...
    meta.ageMin = ageMin;
    meta.interest = interestFor(l.id, meta.hot, rec ? rec.firstSeen : null, meta.priceDropPct, now);
...
module.exports = { recordSnapshot, decorate, trending, stats, historyFor, rememberRecent, recent, interestFor };
